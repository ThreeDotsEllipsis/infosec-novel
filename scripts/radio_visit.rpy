    ...
    if len(menuset) == 8:
        ...
        menu:
            ...
            "6(Есть ли внеурочная деятельность?)":
                jump life
            "7(В чем преимущество данной программы?)":
                jump advantages
            "8(Что умеет универсальный безопасник?)":
                jump ability
...
label life:
    svet "Вне учебная деятельность делится на 7 категорий:
    Спорт, творчество, студенческая наука, международное сотрудничество, волонтерство, лига 
    КВН и студенческий отряд."
    jump questions

label advantages:
    svet "Ну во-первых, высокая востребованность на рынке труда, количество вакансий всегда 
    превышает количество кандидатов. К тому же часть преподавателей – сотрудники компаний, 
    обладающие реальным опытом в сфере."
    serg "Ещё существует сообщество кибербезопасности UCS для дополнительного погружения 
    в сферу и лаборатория кибербезопасности NEO УЦСБ с техническим обеспечением, которое 
    позволяет развертывать системы разной сложности."
    svet "А официальным  партнером программы является ведущий системный интегратор – 
    УЦСБ."
    jump questions

label ability:
    serg "Ну ты даешь! Поступаешь на специальность, про которую ничего не знаешь?"
    svet "Да ладно тебе, сам же таким был. И когда успел стать таким занудой…"
    serg "Что ты сразу такими словами разбрасываешься? Ладно, был не прав, признаю."
    svet "Так бы сразу. Ладно, что-то мы отвлеклись. Итак, универсальный безопасник знает 
    архитектуру сетей, понимает базовые принципы построения безопасных сетей, умеет их 
    защищать."
    serg "Умеет разрабатывать программные продукты на C#, Python, C++;
    может расследовать компьютерные инциденты;"
    serg "Владеет организационно-правовыми основами информационной безопасности;
    умеет использовать математические методы в криптографических задачах."
    svet "А также понимает, как администрировать операционную систему и находить ее 
    уязвимости;
    умеет проводить криминалистический анализ в операционных системах и умеет применять по 
    назначению технические средства защиты."
    mc "Ого. И я всё это буду уметь?"
    svet "Конечно. Главное стараться и верить в себя."
    mc "Я постараюсь, но это звучит так серьезно."
    serg "Вообще не переживай. Если Ден смог, то и ты сможешь."
    mc "Хах, спасибо за поддержку."
    jump questions
